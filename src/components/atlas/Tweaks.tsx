...
const SOUNDS: [TweakState['sound'], string][] = [['off', '无'], ['guqin', '古琴']];
    ...
    <div style={{ position: 'fixed', right: 18, bottom: 18, zIndex: 2147483646, fontFamily: 'var(--font-body)' }}>
      {open && (
        <div style={{
          ...
          <div>
            <span style={label}>背景音 · 古琴留白</span>
            <div style={{ display: 'flex', gap: 4, marginTop: 8, padding: 3, borderRadius: 9, background: 'var(--accent-soft)' }}>
              {SOUNDS.map(([k, lab]) => (
                <button key={k} onClick={() => onChange('sound', k)} style={btn(value.sound === k)}>{lab}</button>
              ))}
            </div>
          </div>
        </div>
