...
1. **（已上线）原型骨架 + 样板内容**：见上表。
2. **内容做厚**：64 卦全卦爻辞 ✓ + 白话 ✓ + 历代易注(程颐 64/64 · 朱熹 63/64，卦辞级) ✓；尚余十翼全文、道德经 81 章、各家其余经典、爻级集注。
3. **检索**：全文检索已上线（跨经文搜原文/白话）；可继续做检索高亮跳到具体句、模糊匹配。
4. **工程化**：接入完整经文库 + 数据库 + 账号云端收藏/进度（开发交接）。

## 内容来源与版权

所列中国经典均属公有领域。**六十四卦原文**由 `scripts/build_hex.mjs` 从公有领域底本汇编：卦辞/爻辞/用九六取自 [openqt/gua](https://github.com/openqt/gua)（简体），彖/大象/小象取自 [bollwarm/ZHOUYI](https://github.com/bollwarm/ZHOUYI)（繁体经 [opencc-js](https://github.com/nk2028/opencc-js) 转简）；并以乾坤泰否既济未济 6 卦做端到端校验。**六十四卦白话**为本项目原创简译（`hex-gloss.ts`）。**历代易注**由 `scripts/build_jizhu.mjs` 取自御纂《周易折中》（清·李光地，公有领域；藏于 [garychowcmu/daizhigev20](https://github.com/garychowcmu/daizhigev20) 殆知阁语料）——该本简体、逐卦标「本义」「程传」，故朱熹《周易本义》、程颐《伊川易传》一并汲取。西方对照仅含书目信息与本项目原创评注，不含任何版权原文。设计稿由 Claude Design 协助生成。

> 注：**历代易注**（卦页「展开历代易注」）为卦辞级——程颐《伊川易传》覆盖 64/64 卦、朱熹《周易本义》覆盖 63/64 卦（仅第 24 卦「复」之本义于折中卦辞处阙如，标「待补」）；爻级集注尚未接入。卦级**简介**沿用底本编辑性描述，个别或有错字，待校订。
