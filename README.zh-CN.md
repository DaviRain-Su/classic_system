...
- **易经（骨干·已做完核心）**：**六十四卦全文**（卦辞·彖·大象·爻辞·小象 全备）+ **全套白话**（卦辞·爻辞 + 彖64·大象64·小象384 逐条简译）+ **爻位义理**（当位/居中 + 承乘比应，由爻推算）+ **历代易注**（程颐/朱熹，卦辞级64+爻级386）+ **十翼全配**：彖/象按卦内嵌，系辞(21段)·文言·说卦·序卦·杂卦 可读互跳，另接入十翼全文底本用于全文阅读与检索；
...
1. **（已上线）原型骨架 + 样板内容**：见上表。
2. **《周易》核心已做完**：64 卦全卦爻辞 ✓ 白话 ✓、彖/大象/小象白话(64+64+384) ✓、爻位承乘比应 ✓、历代易注(卦辞64+爻级386) ✓、十翼精读互跳 + 全文底本 ✓；**道德经 81 章已补齐**；庄子内篇与坛经十品已加厚精选；尚余「做厚」：十翼全文白话/逐句注、道德经历代注、各家其余经典。
3. **检索**：全文检索已上线（跨经文搜原文/白话）；可继续做检索高亮跳到具体句、模糊匹配。
4. **工程化**：接入完整经文库 + 数据库 + 账号云端收藏/进度（开发交接）。

## 内容来源与版权

所列中国经典均属公有领域。**六十四卦原文**由 `scripts/build_hex.mjs` 从公有领域底本汇编：卦辞/爻辞/用九六取自 [openqt/gua](https://github.com/openqt/gua)（简体），彖/大象/小象取自 [bollwarm/ZHOUYI](https://github.com/bollwarm/ZHOUYI)（繁体经 [opencc-js](https://github.com/nk2028/opencc-js) 转简）；并以乾坤泰否既济未济 6 卦做端到端校验。**十翼全文**由 `scripts/build_ten_wings.mjs` 从 [bollwarm/ZHOUYI](https://github.com/bollwarm/ZHOUYI) 抽取并转简，当前全文底本先接入原文阅读/检索，白话与逐句注解继续分层补。**六十四卦白话**为本项目原创简译（`hex-gloss.ts`）。**历代易注**（卦辞级 + 爻级）由 `scripts/build_jizhu.mjs` 取自御纂《周易折中》（清·李光地，公有领域；藏于 [garychowcmu/daizhigev20](https://github.com/garychowcmu/daizhigev20) 殆知阁语料）——该本简体、逐卦逐爻标「本义」「程传」，故朱熹《周易本义》、程颐《伊川易传》一并汲取；折中阙如者（复卦卦辞本义、乾九五爻注）另取自《周易本义通释》《伊川易传》补足。西方对照仅含书目信息与本项目原创评注，不含任何版权原文。设计稿由 Claude Design 协助生成。
