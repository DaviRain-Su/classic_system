...
| **易学讲堂** | 逐课讲解阴阳、加一倍法、八卦取象、解卦语法、十翼、象数义理与群论；新增十二消息卦在 Q6 六维立方体中的透镜形闭环。 |
...
| **多章经文** | 道德经(全 81 章 + 81 条章旨 + 81 条传统注读)/庄子(内篇七篇加厚精选)/六祖坛经(十品加厚精选)：左章目录、右逐句点读。 |
...
| **元会运世盘** | 皇极经世交互：十二消息卦圆图（复居冬至之下、姤居夏至之上，右半「息」左半「消」）+ 「演示阳长阴消」渐变 + 元会运世数表 + 回扣立体图/复卦。 |
...
├─ HuangjiPan.tsx   # 皇极经世 · 元会运世盘
├─ XiaoXiCycle.tsx  # 十二消息卦圆图（皇极页/讲堂复用）
├─ learn/widgets/   # 讲堂交互件（含十二消息卦 Q6 透镜闭环）
├─ cast.tsx         # 起卦
├─ West.tsx         # 西方对照地图
...
- **易经（骨干·已做完核心）**：**六十四卦全文**（卦辞·彖·大象·爻辞·小象 全备）+ **全套白话**（卦辞·爻辞 + 彖64·大象64·小象384 逐条简译）+ **爻位义理**（当位/居中 + 承乘比应，由爻推算）+ **历代易注**（程颐/朱熹，卦辞级64+爻级386）+ **十翼全配**：彖/象按卦内嵌，系辞(21段)·文言·说卦·序卦·杂卦 可读互跳，另接入十翼全文底本用于全文阅读与检索，**十翼全文 128 段白话已覆盖**；
- **道家**（7）：道德经全 81 章 + 81 条章旨 + 81 条传统注读 · 庄子内篇七篇加厚精选 · 列子八篇精选 · 太乙金华宗旨精选 · 常清静经 · 阴符经 · 参同契开篇；
...
1. **（已上线）原型骨架 + 样板内容**：见上表。
2. **《周易》核心已做完**：64 卦全卦爻辞 ✓ 白话 ✓、彖/大象/小象白话(64+64+384) ✓、爻位承乘比应 ✓、历代易注(卦辞64+爻级386) ✓、十翼精读互跳 + 全文底本 ✓，且十翼全文 128 段白话 ✓；**道德经 81 章 + 81 条章旨 + 81 条传统注读已补齐**；庄子内篇、列子八篇、太乙金华宗旨与坛经十品已加厚精选；尚余「做厚」：十翼逐句注/历代注、道德经逐句历代注、各家其余经典。
3. **检索**：全文检索已上线（跨经文搜原文/白话）；可继续做检索高亮跳到具体句、模糊匹配。
4. **工程化**：接入完整经文库 + 数据库 + 账号云端收藏/进度（开发交接）。

## 内容来源与版权

所列中国经典均属公有领域。**六十四卦原文**由 `scripts/build_hex.mjs` 从公有领域底本汇编：卦辞/爻辞/用九六取自 [openqt/gua](https://github.com/openqt/gua)（简体），彖/大象/小象取自 [bollwarm/ZHOUYI](https://github.com/bollwarm/ZHOUYI)（繁体经 [opencc-js](https://github.com/nk2028/opencc-js) 转简）；并以乾坤泰否既济未济 6 卦做端到端校验。**十翼全文**由 `scripts/build_ten_wings.mjs` 从 [bollwarm/ZHOUYI](https://github.com/bollwarm/ZHOUYI) 抽取并转简，白话覆盖层维护在 `src/components/atlas/ten-wing-gloss.ts`，当前已覆盖生成出的十翼全文阅读 128 段。**六十四卦白话**为本项目原创简译（`hex-gloss.ts`），十翼、道儒佛各家白话亦为项目原创整理；《列子》《太乙金华宗旨》等非易经选段取公有领域经典文本做精选阅读。**历代易注**（卦辞级 + 爻级）由 `scripts/build_jizhu.mjs` 取自御纂《周易折中》（清·李光地，公有领域；藏于 [garychowcmu/daizhigev20](https://github.com/garychowcmu/daizhigev20) 殆知阁语料）——该本简体、逐卦逐爻标「本义」「程传」，故朱熹《周易本义》、程颐《伊川易传》一并汲取；折中阙如者（复卦卦辞本义、乾九五爻注）另取自《周易本义通释》《伊川易传》补足。西方对照仅含书目信息与本项目原创评注，不含任何版权原文。设计稿由 Claude Design 协助生成。
