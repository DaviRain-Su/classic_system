...
- remaining full-corpus Ten Wings glosses beyond the completed Wenyan Zhuan layer
...
- `scripts/build_ten_wings.mjs` extracts the complete Ten Wings source text from [bollwarm/ZHOUYI](https://github.com/bollwarm/ZHOUYI), also converted through `opencc-js`; project-written full-corpus glosses are layered separately in `src/components/atlas/ten-wing-gloss.ts`, currently covering the complete Wenyan Zhuan layer plus selected Ten Wings passages elsewhere in the app.
